...
async def fetch_candles(pair):
    ...
    print(f"   - start: {start.isoformat()} ({int(start.timestamp())})", flush=True)
    print(f"   - end: {now.isoformat()} ({int(now.timestamp())})", flush=True)
    print(f"   - granularity: ONE_MINUTE", flush=True)
